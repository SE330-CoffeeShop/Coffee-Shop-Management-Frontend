...
import { columns, statusAdminOptions } from "@/data/order.data";
...
const Orders = () => {
    ...
    const fetchAllOrders = async () => {
      try {
        ...
        while (page <= totalPages) {
          ...
          const response = await fetcher(`/orders/all?${params.toString()}`);
  ...
  const filteredOrders = useMemo(() => {
    ...
    result = result.filter(
      (order: OrderDto) => order.orderStatus === selectedStatus
    );

    // Lọc theo tên khách hàng
    if (filterCustomerName) {
      result = result.filter((order: OrderDto) =>
        order.userName
          ?.toLowerCase()
    ...
    return result;
  }, [allOrders, selectedStatus, filterCustomerName]);
  ...
  const totalPages = useMemo(() => {
    return filteredOrders.length ? Math.ceil(filteredOrders.length / rowsPerPage) : 0;
  }, [filteredOrders]);

  const loadingState = allOrders.length === 0 ? "loading" : "idle";
                    ...
                    >
                      {(columnKey: Key) => (
                        <TableCell className="py-3">
                          {renderCell(item, columnKey, paginatedOrders.indexOf(item))}
                        </TableCell>
                      )}
                    </TableRow>
                  )}
