...
import { CreateDiscountDto } from "@/types/discount.type";
...
const AddDiscountModal = ({
  ...
  const selectedProducts: SelectedProduct[] = products.map((product) => ({
  ...
  useEffect(() => {
    const fetchVariants = async () => {
      ...
      setProductVariantIds(allVariantIds);
    };

    if (Array.from(productIds).length > 0) {
      fetchVariants();
    } else {
  ...
  const handleSubmit = async () => {
    if (validateInputs()) {
      try {
        ...
        console.log("Payload to create discount:", payload);
  ...
  const handleRemoveProduct = (productId: string) => {
    ...
    updatedProductIds.delete(productId);
    ...
    >
      <ModalContent>
        <ModalHeader className="w-full rounded-t-xl border">
          ...
          <div className="ml-5">
            <div className="text-lg font-semibold">Thêm khuyến mãi</div>
            <div className="text-wrap text-sm font-normal">
              Tạo và quản lý chương trình khuyến mãi dễ dàng
            </div>
          </div>
        </ModalHeader>
        <ModalBody>
                  ...
                  <Input
                    ...
                    endContent={
                      <span className="text-sm text-gray-500">
                        {discountTypeMap[selectedDiscountType] === "PHẦN TRĂM" ? "%" : "VNĐ"}
                      </span>
                    }
